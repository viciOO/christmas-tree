...
1. 登录 [Vercel](https://vercel.com/) 并导入本仓库（`Add New Project` → `Import Git Repository`）。
2. 默认构建命令 `npm run build`，产物目录 `dist`。无需额外设置即可识别 Vite。
3. `vite.config.ts` 已根据 `VERCEL` 环境变量自动切换 `base`，在 Vercel 上会使用根路径 `/`，在 GitHub Pages 上则保持 `/christmas-tree/`。
4. 首次部署完成后会获得 `*.vercel.app` 域名，可在项目 `Settings → Domains` 添加自定义域。
5. 每次向 GitHub 推送代码，Vercel 会自动安装依赖、构建并发布，无需 GitHub Actions。
